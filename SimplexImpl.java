...
dd
	...
	public static class Modeler {
		...
		public Modeler(double[][] constraintLeftSide, double[] constraintRightSide, Constraint[] constraintOperator,
			...
			/**
			* Ax = b : A(numberOfConstraints , numberOfOriginalVariables + numberOfConstraints) & b(numberOfConstraints)
			* Cx = z : C(numberOfOriginalVariables)
			*
			* 
			**/

			// initialize constraint - A(variables-hors-base)
			for (int i = 0; i < numberOfConstraints; i++) {
				for (int j = 0; j < numberOfOriginalVariables; j++) {
					a[i][j] = constraintLeftSide[i][j];
				}
			}
			// A(variables-de-base: Identité ou smth else)
			for (int i = 0; i < numberOfConstraints; i++)
	...
	public static void main(String[] args) {
		...
		double[] objectiveFunc = { 3, 2 }; // =  Z = 3.X_1 + 2.X_2
		double[][] constraintLeftSide = { { 1, 2 }, { 2, 1 }, { -1, 1 }, { 0, 1 } }; // = Aj(varOrginal)
		Constraint[] constraintOperator = { Constraint.lessThan, Constraint.lessThan, Constraint.lessThan,
				Constraint.lessThan };

		double[] constraintRightSide = { 6, 8, 1, 2 };// =b
		// X_1 + 2.X_2 <= 6 , 2.X_1 + 1.X_2 <= 8, ....
